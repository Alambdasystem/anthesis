...
def create_lecture():
    ...
    try:
        ...
        try:
            ...
            lecture_content = response_data['message']['content']
...
def get_quizzes(track):
    """Get all quizzes for a track."""
    try:
        quizzes = load_quizzes()
        track_quizzes = quizzes.get(track, [])
        return jsonify(track_quizzes)
    except Exception as e:
        logging.error(f"Error getting quizzes: {e}")
        return jsonify({"error": str(e)}), 500
