...
app.secret_key = bytes(config['secret-key'], 'utf-8')
...
def sign_for_course(id_user,id_course,course_name,progress,difficulty,requiremment,img):
    sql="INSERT INTO Lesson_inscrit(id_user,id_course,course_name,progress,difficulty,requiremment,image) VALUES (%s,%s,%s,%s,%s,%s,%s)"
    data=(id_user,id_course,course_name,progress,difficulty,requiremment,img)
...
def get_first_chapitre(id_course):
    ...
    sign_for_course(user_id,id_course,result.encode('utf-8'),0,get_difficulty(id_course),get_requiremment(id_course).encode('utf-8'),img)
...
def backup():
    DB_NAME = 'teach_me'
    BACKUP_PATH = r'D:\Data\pfe'
    DB_HOST = 'localhost' 
    DB_USER = 'user'
    DB_USER_PASSWORD ='Root@293#c'
    ...
    dumpcmd = (
        ...
        " -p" + DB_USER_PASSWORD +
        " " + db +
        " > " + shlex.quote(TODAYBACKUPPATH + "/" + db + ".sql")
    )
    os.system(dumpcmd)
    gzipcmd = "gzip " + shlex.quote(TODAYBACKUPPATH + "/" + db + ".sql")
    os.system(gzipcmd)
...
def decode_charts(charts):
    decoded_charts = []
    for chart in charts:
        decoded_chart = []
        for item in chart:
            decoded_chart.append(decode_value(item))
        decoded_charts.append(tuple(decoded_chart))
    return decoded_charts

def decode_courses(courses):
    decoded_courses = []
    for course in courses:
        decoded_course = [decode_value(item) for item in course]
        decoded_courses.append(decoded_course)
    return decoded_courses

def generate_random_datetimes(start_date, end_date, length):
    """
    Generate a list of random datetime objects.

    :param start_date: The start date (datetime object) for the range.
    :param end_date: The end date (datetime object) for the range.
    :param length: The number of random datetime objects to generate.
    ...
    for _ in range(length):
        ...
        random_datetimes.append(random_date)
...
def EditProfile():
    ...
    if request.method == "POST":
        ...
        if request.files and imgg:
            ...
            buffer.seek(0)
            encodedimg=base64.b64encode(buffer.read()).decode('utf-8')
          
            sql="UPDATE Users SET Name=%s , Surname=%s , Email=%s , Birth_date=%s, photo=%s  WHERE ID=%s"
            data=(name,surname,email,birth,encodedimg,user_id)
            cursor.execute(sql,data)
            conn.commit()
        else:
         
            sql="UPDATE Users SET Name=%s , Surname=%s , Email=%s , Birth_date=%s WHERE ID=%s"
            data=(name,surname,email,birth,user_id)
            cursor.execute(sql,data)
            conn.commit()
...
def MyCourses():
    ...
    for i, sublist in enumerate(signed_up_courses2):
        if sublist[2] == '0':
            sql="SELECT MIN(id) FROM chapitres WHERE course_id='{}'".format(sublist[0])
            cursor.execute(sql)
            progress=cursor.fetchall()[0]
            ex_value=progress[0]
            progress=ex_value
            sublist[2]=str(progress)
...
def forum():
    ...
    if request.method =="POST":
        ...
        sql="INSERT INTO Forum_Question (author,subject,content,time) VALUES('{}','{}','{}','{}')".format(username,subject,content,datetime.datetime.now())
        cursor.execute(sql)
        conn.commit()
...
def forum_question():
    ...
    role=get_Role()
    username=get_user_name(get_user_id(request.cookies.get('sessionid')))
    sql="SELECT subject, content, id, author,time FROM Forum_Question"
    cursor.execute(sql)
    all_question=cursor.fetchall()
    x=list()
    all_ids=list()
    for i in all_question:
        x.append(tuple(map(str, i)))
        all_ids.append(i[2])
    print('====',all_ids)

    no_of_comments=list()
    for i in all_ids:
            sql="SELECT COUNT(id) FROM comment WHERE id_post='{}'".format(i)
            cursor.execute(sql)
            no_of_comments.append(cursor.fetchall()[0][0])

    ques = list(zip(x, no_of_comments))

    return render_template("forum_question.html",all_question=ques,username=username,role=role)
...
def Create_course():
    ...
    if request.method=="POST":
        ...
        if request.files:
            ...
            sql="INSERT INTO Courses(title,introduction,requirement,difficulty,image) VALUES(%s,%s,%s,%s,%s)"
            data=(title,intro,requiremment,difficulty,encodedimg)
            cursor.execute(sql,data)
            conn.commit()
        else:
            sql="INSERT INTO Courses(title,introduction,requirement,difficulty) VALUES('{}','{}','{}','{}')".format(title,intro,requiremment,difficulty)
            cursor.execute(sql)
            conn.commit()
    return render_template("Create_course.html",username=username,role=role)
...
def editor():
    ...
    if request.method=="POST":
        ...
        content=request.form["content"]
        username=get_user_name(get_user_id(request.cookies.get('sessionid')))
        sql="INSERT INTO Forum_Question (author,subject,content,time) VALUES ('{}','{}','{}','{}')".format(username,title,content,datetime.datetime.now())
        cursor.execute(sql)
        conn.commit()
...
def Add_chapitre(id):
    ...
    if request.method=="POST":
        ...
        sql="INSERT INTO chapitres(title,content,Course_id) VALUES(%s,%s,%s)"
...
def Current_chapiter(id,id_chap):
    ...
    # print("last_chapitre",last_chapitre[0])
    Current_chapiter_id= int(request.url.split('/')[8])
    if Current_chapiter_id==0:
        sql="SELECT MIN(id) FROM chapitres WHERE course_id='{}'".format(id)
        cursor.execute(sql)
        Current_chapiter_id=cursor.fetchall()[0]
        ex_value=Current_chapiter_id[0]
        Current_chapiter_id=ex_value
...
def Add_quizz(id):
    ...
    if request.method=="POST":
        ...
        sql="INSERT INTO quizz(name,content,id_course,id_user,correct_answer) VALUES(%s,%s,%s,%s,%s)"
        data=(title,quiz,id_course,user_id,good_answers)
        cursor.execute(sql,data)
        conn.commit()
...
def Felicitation(id):
    ...
    if request.method=="POST":
        os.system("magick sample-out.jpg cert.pdf")
        f=open("cert.pdf","r")
        return send_file("./cert.pdf", as_attachment=True,download_name="cert.pdf")
...
def Add_Events():
    ...
    if request.method=="POST":
        ...
        content=request.form["content"].encode('ascii','ignore')
        content=content.decode().encode('utf-8')
        sql="INSERT INTO Events(name,content,id_user) VALUES(%s,%s,%s)"
        data=(title,content,get_user_id(request.cookies.get("sessionid")))
        cursor.execute(sql,data)
        conn.commit()    
...
def Event(id):
    ...
    link_to_stream = "http://" + event_info[0][2][event_info[0][2].find("192.168"):]

    return render_template("Event.html",event_info=event_info,link_to_stream=link_to_stream,login_button=login_button,username=username)
